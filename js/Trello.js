  ...
  wrapper = function(window, jQuery, opts) {
    ...
    Trello = {
      ...
      authorize: function(userOpts) {
        var k, persistToken, ref, regexToken, returnUrl, scope, v;
        opts = $.extend(true, {
          type: "redirect",
          persist: true,
          interactive: true,
        ...
        switch (opts.type) {
          case "popX":
            returnUrl = chrome.extension.getURL(opts.token_html);
            chrome.windows.create({
              url: "https://trello.com/1/authorize?" + "response_type=token" + "&key=" + opts.key + "&return_url=" + encodeURI(returnUrl) + "&scope=read,write,account&expiration=never" + "&name=" + opts.name,
              width: 520,
              height: 620,
              type: "panel",
              focused: true
          ...
          case "popup":
            (function() {
              var authWindow, height, left, origin, receiveMessage, top, width;
              ...
              origin = window.location.href.substring(0, window.location.href.lastIndexOf('/')) + '/' + opts.token_html;
